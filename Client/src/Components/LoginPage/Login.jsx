...
const Login = () => {
  ...
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [isOtpPage, setIsOtpPage] = useState(false); // OTP step toggle
  const inputRefs = useRef([]);
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (state === "Sign Up") {
        ...
        // Login - Request OTP
        const { data } = await axios.post(`${backendurl}/api/auth/login-request-otp`, { email, password });

        if (data.success) {
          toast.success("OTP sent! Enter to continue.");
          setIsOtpPage(true);
        } else {
          toast.error(data.message);
        }
      ...
      <div className="bg-white p-6 sm:p-10 rounded-lg shadow-lg w-full sm:w-100 xl:w-[35%] text-black-300 text-sm">
        ...
        {isOtpPage ? (
          ...
          <form onSubmit={onSubmitHandler}>
            ...
            <p onClick={() => navigate("/Reset_pass")} className="text-black cursor-pointer hover:text-blue-700">
              Forgot Password?
            </p>
            <button className="w-full !py-3 rounded mt-1 bg-[#fb541b] text-white font-medium">
              {state}
        ...
        {!isOtpPage && (
          <p className="text-black-400 text-center text-ls mt-4">
            {state === "Sign Up" ? (
              <>
                Already have an account?{" "}
                <span onClick={() => setState("Login")} className="text-blue-700 cursor-pointer underline">
                  Login here
                </span>
              </>
            ) : (
              <>
                Don't have an account?{" "}
                <span onClick={() => setState("Sign Up")} className="text-blue-700 cursor-pointer underline">
                  Sign up here
                </span>
              </>
            )}
